...
function getDateRange(id: string): { start: Date, end: Date } {
  const year = 2026;
  if (id.includes("-")) {
    const [startStr, endStr] = id.split("-");
    const startMatch = startStr.match(/([a-z]+)(\d+)/);
    const endMatch = endStr.match(/([a-z]+)(\d+)/);
    if (startMatch && endMatch) {
       const startMonth = new Date(Date.parse(startMatch[1] +" 1, 2026")).getMonth();
       const endMonth = new Date(Date.parse(endMatch[1] +" 1, 2026")).getMonth();
       return {
         start: new Date(year, startMonth, parseInt(startMatch[2])),
         end: new Date(year, endMonth, parseInt(endMatch[2]), 23, 59, 59, 999)
       };
    }
  } else if (id.match(/^[a-z]+\d{4}$/)) {
    const monthStr = id.substring(0, 3);
    const yearStr = id.substring(3);
    const month = new Date(Date.parse(monthStr + " 1, 2026")).getMonth();
    return {
      start: new Date(parseInt(yearStr), month, 1),
      end: new Date(parseInt(yearStr), month + 1, 0, 23, 59, 59, 999)
    };
  } else {
...
function BestPageContent() {
  ...
  const initMode = searchParams.get("mode") || "weekly";
  const initDate = searchParams.get("date") || MODE_RANGES[initMode][0].id;

  const [dbProducts, setDbProducts] = useState<Product[]>([]);
  const [mode, setMode] = useState(initMode);
  const [activeRange, setActiveRange] = useState(initDate);
  ...
  // Update activeRange when mode changes by user click
  useEffect(() => {
    // Only update activeRange if the current mode doesn't match the initMode when component first mounts,
    // or if the mode change isn't from the URL. For simplicity, just update it if the current activeRange
    // isn't valid for the new mode.
    if (!MODE_RANGES[mode].some(r => r.id === activeRange)) {
